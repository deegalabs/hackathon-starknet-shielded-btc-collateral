...
cat > "$DEPLOYMENT_DIR/sepolia.json" <<EOF
{
  "network": "sepolia",
  "timestamp": "$TIMESTAMP",
  "deployer": "$DEPLOYER",
...
ok "Deployment manifest saved to scripts/deployment/sepolia.json"

# ── Generate .env snippet ──────────────────────────────────────────────────────
cat > "$DEPLOYMENT_DIR/frontend.env.sepolia" <<EOF
...
ok "Frontend env saved to scripts/deployment/frontend.env.sepolia"
log "Copy to frontend: cp scripts/deployment/frontend.env.sepolia frontend/.env"
...
echo ""
echo "  Next steps:"
echo "    1. cp scripts/deployment/frontend.env.sepolia frontend/.env"
